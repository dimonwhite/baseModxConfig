...
if ($q->prepare() && $q->stmt->execute()) {
    ...
    array (
      ...
      'category' => '0',
      'cache_type' => '0',
      'plugincode' => 'if ($modx->event->name == \'OnLoadWebDocument\' && request()->isAjax() && request()->checkCsrfToken()) {

    $mailsTo = \'no-reply@sevds.example-site.ru\';
    $emailSendler = $modx->config[\'emailsender\'];
    
    if ($_POST[\'call\']) {
        
        $modx->runSnippet(\'Formit\', [
            \'hooks\' => \'FormItSaveForm,spam,email\',
            \'emailFrom\' => $emailSendler,
            \'emailTo\' => $mailsTo,
            \'emailSubject\' => \'[[+marker]]: [[+name]] ([[+phone]])\',
            \'emailTpl\' => \'email.tpl\',
            \'validate\' => \'name:required,phone:required,lastname:blank\',
        ...
        if ($modx->getPlaceholder(\'fi.validation_error_message\')) {
            $response = [\'success\' => \'error\', \'errors\' => $modx->getPlaceholder(\'fi.validation_error_message\')];
    ...
    array (
      ...
      'category' => '0',
      'cache_type' => '0',
      'plugincode' => '$manifest = MODX_ASSETS_PATH . \'templates/manifest.json\';
if (file_exists($manifest)) {
    $files = json_decode(file_get_contents($manifest), true);
    foreach ($files as $file) {
        if (strpos($file, \'.css\') && !strpos($file, \'.map\')) {
            $modx->regClientCSS(\'assets/templates/\'.$file);
        } elseif (strpos($file, \'.js\') && !strpos($file, \'.map\')) {
            $modx->regClientScript(\'assets/templates/\'.$file);
        }
    }
}',
      'locked' => '0',
    ...
    array (
      ...
      'source' => '1',
      'property_preprocess' => '0',
      'name' => 'toLowerRedirect',
      'description' => '',
      'editor_type' => '0',
      'category' => '0',
      'cache_type' => '0',
      'plugincode' => 'if ($modx->context->get(\'key\') == \'mgr\') {return;}
